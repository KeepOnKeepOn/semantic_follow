...
public:
    ...
    camera2scan_yaw_(0), dist_camera_(0), dist_scan_(0), last_update_(ros::Time::now()),reference_(1.5){
        ...
        while(!buffer_->canTransform("base_scan", camera_frame_id, ros::Time(0))){
            ROS_INFO("Wait for transform from camera_link to base_scan... \n");
            ros::Duration(0.5).sleep();
        }
        geometry_msgs::TransformStamped camera2scan = buffer_->lookupTransform(camera_frame_id, "base_scan", ros::Time(0));
